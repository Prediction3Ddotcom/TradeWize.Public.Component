...
export default class DaysGridView extends Component {
  constructor(props) {
    ...
    this.initMonthSettings = params => {
      ...
      const startIndex = (firstDay > 0 ? firstWeekDay + (Utils.FIRST_DAY_OFFSETS[firstDay] || 0) : firstWeekDay) % 7;
      return {
        maxWeekRows: 6,
        numDaysInWeek: 7,
        numDaysInMonth,
  ...
  componentDidUpdate(prevProps) {
    ...
    if (propDiffs.length) {
      ...
      const {
        firstDayOfMonth
      } = this.state.monthSettings;
      const isSelectedDiff = !Utils.compareDates(selectedStartDate, prevSelStart, 'day') || !Utils.compareDates(selectedEndDate, prevSelEnd, 'day');
  ...
  renderEmptyDay(key) {
    return {
      component: /*#__PURE__*/_jsx(EmptyDay, {
  ...
  render() {
    ...
    const renderedDaysGrid = daysGrid.map((weekRow, i) => /*#__PURE__*/_jsx(View, {
      style: styles.weekRow,
      children: weekRow?.map(day => day.component)
    }, i));
...
//# sourceMappingURL=DaysGridView.js.map
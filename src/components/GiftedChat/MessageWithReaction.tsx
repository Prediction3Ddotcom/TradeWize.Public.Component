...
export const MessageWithReaction = ({
  ...
  const getSafeHorizontalPosition = useCallback(
    (basePosition: number, elementWidth: number) => {
  ...
  const styleMessage = useCallback(() => {
    ...
    if (isMyMessage) {
      ...
      const baseRight = 16;
      const safeRight = getSafeHorizontalPosition(
        screenWidth - baseRight - messageWidth,
        messageWidth
      );
      return {
        top,
  ...
  const styleReactionIcon = useCallback(() => {
    ...
    if (isMyMessage) {
      const baseRight = 16;
      const safeRight = getSafeHorizontalPosition(
        screenWidth - baseRight - iconWidth,
        iconWidth
      );
      return {
        top,
  ...
  const styleLayoutAction = useCallback(() => {
    ...
    if (isMyMessage) {
      const baseRight = 16;
      const safeRight = getSafeHorizontalPosition(
        screenWidth - baseRight - actionWidth,
        actionWidth
      );
      return {
        top,
  ...
  const onLayout = useCallback(
    (e: LayoutChangeEvent) => {
      ...
      const isHeight =
        position.pageY + height + position.height + 139 > screenHeight;

      setDifferenceLevel(
        position.pageY + height + position.height - screenHeight
      );

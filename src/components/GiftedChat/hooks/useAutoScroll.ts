...
export function useAutoScroll<TMessage extends IMessage = IMessage>({
  messages,
  inverted,
  isTyping,
  isLoadingEarlier,
  ...
  const isFirstRender = useRef({ scrollToBottom: true });
  ...
  const scrollTo = useCallback(
    (options: { animated?: boolean; offset: number }) => {
      if (forwardRef?.current && options) {
        if (useScrollView) {
          const scrollViewRef = forwardRef.current as any;
    ...
    (animated: boolean = true) => {
      if (forwardRef?.current) {
        if (useScrollView) {
          ...
          if (inverted) {
            if (scrollViewRef && 'scrollTo' in scrollViewRef) {
              scrollViewRef.scrollTo({ x: 0, y: 0, animated });
            }
          } else {

...
import { MIN_COMPOSER_HEIGHT, MAX_COMPOSER_HEIGHT } from '../Constant';
...
export function useGiftedChat<TMessage extends IMessage = IMessage>(
  ...
  const textInputRef = useMemo(
    () => props.textInputRef || createRef<any>(),
    [props.textInputRef]
  );
  ...
  const scrollToBottom = useCallback(
    (isAnimated = true) => {
      ...
      if (
        ...
        const scrollViewRef = messageContainerRef.current as any;
        if (inverted) {
          scrollViewRef.scrollTo({ y: 0, animated: isAnimated });
        } else {
  ...
  const onPressPickMedia = useCallback(
    async (type: 'camera' | 'pick') => {
      if (type === 'camera') {
        ...
        const result = await ImageCropPicker.openPicker({ multiple: true });
  ...
  const onPressFileInContainer = useCallback(
  ...
  const onVideoRecorded = useCallback(
    async (video: any) => {
